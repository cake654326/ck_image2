	...
	function onChangeSrc() {
		...
		if ( value !== widget.data.src ) {
			...
			preLoader( value, function( image, width, height ) {
				...
				// console.dir( editor.config.image2_prefillDimensions );
				
				

				if( editor.config.cxDefaultWidth === false ){
					...
					var _width = editor.config.cxDefaultWidth;
					var _height = parseInt(parseFloat( height ) * parseFloat( _width ) / parseFloat( width ) );
					widthField.setValue( _width);
					heightField.setValue( _height );
				}
				...
				// console.dir( helpers.checkHasNaturalRatio( image ) );
